...
def features_importance(clf, X_train_columns, plot=False, _coef=False):
    if plot == False:
        if _coef:
...
def one_hot_shap_values(X, shap_values):
    ...
    for i in range(len(cat_features) - 1):
        shap_cat = np.sum(shap_values.values[:, features_index[cat_features[i]]:features_index[cat_features[i + 1]]], axis=1)
        features_shap_values[cat_features[i]] = shap_cat
    shap_cat = np.sum(shap_values.values[:, features_index[cat_features[len(cat_features) - 1]]:shap_values.values.shape[1]], axis=1)
